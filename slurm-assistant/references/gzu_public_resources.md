...
```bash
uv run python "$SCRIPT" exec -c "which pdflatex"
```

2. **如果未安装，引导用户使用集群提供的安装脚本**

```bash
uv run python "$SCRIPT" exec -c "sh /home/share/Official/tools/texlive/install.sh"
```
...
```bash
uv run python "$SCRIPT" exec -c "which pdflatex && pdflatex --version"
```

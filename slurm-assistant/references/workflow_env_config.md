...
- 本文讨论的是用户项目环境
- `slurm-assistant` 自身 runtime 已经不依赖 Python
- 文中的 `python`、`uv run python`、`conda` 都是在为用户作业准备环境
...
| 工具 | 负责内容 | 使用场景 |
|------|---------|---------|
| **uv（默认）** | 用户 Python 包管理、工具安装 | 大部分场景 |
| **conda（按需）** | CUDA、C 编译器、底层系统库 | 节点配置不符合需求时 |

**核心原则：**
- **优先使用 uv**：集群节点已配置 CUDA、编译器等环境，uv 安装用户 Python 包即可

...
### Step 0：优先使用 Rust client

优先使用：

```bash
CLIENT="/absolute/path/to/slurm-assistant/rust/target/debug/slurm-client"
SERVER="/absolute/path/to/slurm-assistant/rust/target/debug/slurm-server"
```

如果未构建，可直接通过 Cargo 运行：

```bash
cargo run --quiet --bin slurm-client -- --help
```

只有当 Rust client 尚未覆盖目标能力时，才回退到旧版：
...
#### A. `slurm-client server status` 成功
...
- 如需连接管理，先执行：
  ...
  ```
- `0` 个连接  
  → 走首次配置，读取 `references/workflow_init.md`
- `1` 个连接  
  → 直接使用该连接的 `connection_id`
- 多个连接  
  → 先列出连接，再按用户意图选择 `connection_id`

#### B. Rust server/client 不可用
...
2. **作业管理**
   ...
   - 优先命令：
     ```bash
     cargo run --quiet --bin slurm-client -- submit <脚本> --connection <connection_id> --json
     ...
     cargo run --quiet --bin slurm-client -- alloc -p <分区> --connection <connection_id>
     cargo run --quiet --bin slurm-client -- run --connection <connection_id> -- <命令>
     ```
...
3. **文件传输**
   ...
   - 优先命令：
     ```bash
     cargo run --quiet --bin slurm-client -- upload <本地> <远程> --connection <connection_id> --json
     cargo run --quiet --bin slurm-client -- download <远程> <本地> --connection <connection_id> --json
     ```
...
6. **任意远程命令**
   - 只有当现有子命令覆盖不了用户需求时，才使用：
     ```bash
     cargo run --quiet --bin slurm-client -- exec --connection <connection_id> --cmd '<命令>' --json
     ```
   - 执行前必须做安全分类，见下文“安全分流”。
...
# 添加新连接
cargo run --quiet --bin slurm-client -- connection add --label <名称> --host <IP> --port <端口> --user <用户名> --kind <cluster|instance|server>
...
2. 根据用户意图判断目标连接：
   - 用户说"集群" → 查找 `kind=cluster` 的连接
   - 用户说"实例" → 查找 `kind=instance` 的连接
   - 用户提到端口号（如"21810"）→ 查找对应端口的连接

3. 在后续命令中显式传 `--connection <connection_id>`：
   ```bash
   cargo run --quiet --bin slurm-client -- find-gpu --connection conn_gzu_cluster
   cargo run --quiet --bin slurm-client -- exec --connection conn_gzu_instance_21810 --cmd "hostname"
   ```
...
| 类别 | 命令 | 说明 | 详细 |
|------|------|------|------|
| 状态 | status | 查看资源状态（当前 Rust 版支持 `--gpu`） | `references/commands.md` |
...
| 作业 | run | 使用 `srun` 运行命令 | `references/commands.md` |
| 作业 | release | 释放交互式资源 | `references/commands.md` |
| 作业 | submit | 提交作业 | `references/commands.md` |
| 作业 | jobs | 查看作业状态 | `references/commands.md` |
| 作业 | log | 查看作业日志 | `references/commands.md` |
| 作业 | cancel | 取消作业 | `references/commands.md` |
| 文件 | upload | 上传文件/目录 | `references/commands.md` |
| 文件 | download | 下载文件/目录 | `references/commands.md` |
| 连接 | connection | 管理连接（add/list/get/remove） | `references/commands.md` |
| **核心** | **exec** | **执行远程命令（统一兜底入口）** | `references/commands.md` |
